...
public class HUTraceEventsServiceTests
{
	...
	public void createAndAddEventsWithTwoLUs()
	{
...

		{
			...
			Mockito.doReturn(ImmutableList.of(huAssignment11, huAssignment12))
					.when(huAccessService)
					.retrieveHuAssignments(user1);
			Mockito.doReturn(ImmutableList.of(huAssignment21, huAssignment22, huAssignment22double))
					.when(huAccessService)
					.retrieveHuAssignments(user2);
			...
			Mockito.doReturn(Optional.of(ImmutablePair.of(prod12, qty12))).when(huAccessService).retrieveProductAndQty(vhu12);
			Mockito.doReturn(Optional.of(ImmutablePair.of(prod21, qty21))).when(huAccessService).retrieveProductAndQty(vhu21);
			Mockito.doReturn(Optional.of(ImmutablePair.of(prod22, qty22))).when(huAccessService).retrieveProductAndQty(vhu22);
		}
