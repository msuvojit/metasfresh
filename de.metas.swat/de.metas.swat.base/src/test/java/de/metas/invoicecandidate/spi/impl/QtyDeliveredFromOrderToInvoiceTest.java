...
public class QtyDeliveredFromOrderToInvoiceTest
{
	...
	public void init()
	{
...

		{
			...
			Mockito.when(productAcctDAO.retrieveActivityForAcct(clientId, orgId, productId))
	...
	public void testQtyDelivered()
	{
		...
		assertThat(invoiceCandidates).hasSize(1);

		final I_C_Invoice_Candidate candidate = invoiceCandidates.get(0);
		saveRecord(candidate);
		olHandler.setDeliveredData(candidate);
		assertThat(candidate.getQtyDelivered()).isEqualByComparingTo(orderLine.getQtyDelivered());
	}
}
